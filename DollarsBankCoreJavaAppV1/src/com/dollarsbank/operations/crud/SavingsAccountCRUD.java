...
public class SavingsAccountCRUD implements AccountOps<Account>{
	@Override
	public Account get(int custId) {
		Account a = new SavingsAccount();
		try {
			PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement("SELECT * FROM accounts WHERE customerId=? AND visible=1 AND type='SAVINGS'");
			ps.setInt(1, custId);
	...
	public Vector<Account> getAll(int custId) {
		Vector<Account> av = new Vector<>();
		try {
			PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement("SELECT * FROM account WHERE visible=1 AND type='SAVINGS' AND customerId=?");
			ps.setInt(1, custId);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Account a = new SavingsAccount();
	...
	public void create(Account t) {
		try {
			...
			ps.setObject(3, AccountType.SAVINGS.toString());
			ps.execute();
		} catch(SQLException e) {
