...
public class CheckingTransactionCRUD implements TransactionOps<Transaction>{
	@Override
	...
	public Vector<Transaction> getAll() {
		...
		try {
			ResultSet rs = ConnectionFactory.getConnection().createStatement().executeQuery("SELECT * FROM transaction WHERE type='CHECKING' ORDER BY id DESC LIMIT 25");
			while(rs.next()) {
				Transaction a = new CheckingTransaction();
				a.setId(rs.getInt("id"));
				a.setUserId(rs.getInt("customerTransactionId"));
	...
	public Vector<Transaction> getAll(int custId) {
		Vector<Transaction> tv = new Vector<>();
		try {
			PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement("SELECT * FROM transaction WHERE type='CHECKING' AND customerTransactionId=? ORDER BY id DESC LIMIT 25");
			ps.setInt(1, custId);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Transaction a = new CheckingTransaction();
